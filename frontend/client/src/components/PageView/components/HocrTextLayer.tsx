...
import { Layer, Image, Rect, Text } from "react-konva";
...
export default function HocrLayer({
	...
	const RenderWrdFunction = (
		...
		const wordsEl = wordsArray.map((lineChild) => (
			<Rect
				...
				key={lineChild.id}
				x={parseInt(
					(lineChild.bbox?.x0 * width) /
						pageImage?.orgWidth
				)}
				y={parseInt(
					(lineChild.bbox?.y0 * height) /
						pageImage?.orgHeight
				)}
				width={
					((lineChild.bbox.x1 -
	...
	const RenderWrdTxtFunction = (
		...
		const wordsTextEl = wordsArray.map((lineChild) => (
			<Text
				text={lineChild.text}
				id={lineChild.id}
				ref={(el) => {
