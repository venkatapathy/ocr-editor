...
function parseLine(element: Element): Line {
	...
	return {
		...
		children: Array.from(element).map(parseLine),
		text: element.textContent ?? "",
	};
}
...
function parsePage(doc: Document): Page {
	...
	const lineel = doc.querySelectorAll(".ocr_par");
...
export default function doTranslate(hocrurl): Promise<Page> {
